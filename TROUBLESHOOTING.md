...
## Worker container dies silently mid-render (no exception logged, arq retries as try=2)
Cause: The Clipping tool's render call sites (`VideoService.create_single_clip`, `apply_broll_to_rendered_clip`, `create_video_clips` in `video_service.py`) never acquired the `"gpu"` resource slot — only `ai.py`'s Ollama call and `ranking_service.py`'s render did. With `WorkerSettings.max_jobs = 4`, multiple Clipping tasks can render concurrently with zero VRAM serialization on a small GPU (e.g. a 6GB card), which can hard-crash the whole worker process (OOM-kill/segfault, not a catchable Python exception) rather than just failing the one ffmpeg call. Confirmed by tracing every `resource_slot("gpu", ...)` call site in the codebase (`grep -rn 'resource_slot(' backend/src`) — Clipping's three render sites were missing from that list.
Fix: Already handled — all three Clipping render call sites now wrap their `run_in_thread(...)` call in `async with resource_slot("gpu", 1):`, matching the pattern `ranking_service.py` already used. If it recurs, check whether a newer render call site (e.g. a future tool) skipped this.
